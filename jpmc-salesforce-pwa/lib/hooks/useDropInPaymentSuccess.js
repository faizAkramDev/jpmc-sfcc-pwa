...
export function useDropInPaymentSuccess({
    ...
    const handlePaymentSuccess = useCallback(async (jpmcPayload) => {
        ...
        try {
            ...
            // Add Drop-in mode identifier for order tracking
            const confirmPayload = {
                ...normalizedPayload,
                jpmcCheckoutMode: 'DROP_IN'  // Identifies this as a Drop-in order (vs PIE, Apple Pay, Google Pay)
            }

            // For 3DS deferred auth: Set explicit placeholder for transactionId
            // SFRA sets 'PENDING_3DS' placeholder for deferred cases (see JPMC.js line 234)
            // This allows the notifications job to find and update the order when payment confirmation arrives
            // Placeholder is used as: c_jpmcGatewayTransactionId = 'PENDING_3DS' on order, TransactionID = 'PENDING_3DS' on payment instrument
            // NOTE: This is DROP-IN SPECIFIC - other payment methods (if added in future) won't use this placeholder
            if (isDeferred && !confirmPayload.transactionId && !confirmPayload.paymentGatewayTransactionId) {
                confirmPayload.transactionId = 'PENDING_3DS'
            }
